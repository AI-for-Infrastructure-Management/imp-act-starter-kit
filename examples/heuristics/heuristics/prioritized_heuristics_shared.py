...
class PrioritizedHeuristic(Heuristic):
    ...
    def get_policy_params(self, rules):
        ...
        default_rule_values = rules[-1][1:]  # Ensure we extract rule values correctly
        policy_params = {
            key: threshold
            for key, threshold in zip(self.default_rules_range.keys(), default_rule_values)
        }
    
        # Extract the shared rule set for prioritized components
        shared_prioritized_rule_values = rules[0][1:]  # Ensure correct indexing
    
        # Apply the shared rule set to all prioritized components
        shared_prioritized_rules = {
            key: threshold
            for key, threshold in zip(self.prioritized_rules_range.keys(), shared_prioritized_rule_values)
        }
    
        for c in self.prioritized_components:
            policy_params[c] = shared_prioritized_rules  # Assign the same rule set to all prioritized components
    ...
    def get_policy_space(self):
        p_rules_range_dimensions = [len(rule) for rule in self.prioritized_rules_range.values()]
        np_rules_range_dimensions = [len(rule) for rule in self.default_rules_range.values()]
    
        # Compute the number of possible rule sets
        num_prioritized_rules = math.prod(p_rules_range_dimensions)  # Now only one shared set
        num_default_rules = math.prod(np_rules_range_dimensions)
        num_rules = num_prioritized_rules * num_default_rules  # Final number of rules
    
        print(f"Prioritized rules range dimensions: {p_rules_range_dimensions} ({num_prioritized_rules})")
        print(f"Default rules range dimensions: {np_rules_range_dimensions} ({num_default_rules})")
        print(f"Number of rules: {num_rules} ({num_prioritized_rules} x {num_default_rules})")
    
        # Create shared prioritized rule sets (one set for all prioritized components)
        shared_prioritized_rules = itertools.product(*self.prioritized_rules_range.values())
    
        # Combine with default rules
        default_rules = itertools.product(*self.default_rules_range.values())
    
        # Initialize policy space
        policy_space = []
        for prio_rules, def_rules in itertools.product(shared_prioritized_rules, default_rules):
            # Create a single tuple representing the rules (using prio_rules for all prioritized components)
            rules = [(None, *def_rules)]  # Default rules
            rules.insert(0, (self.prioritized_components, *prio_rules))  # Shared prio rules
            
            policy_params = self.get_policy_params(rules)
            policy = PrioritizedPolicy(policy_params, self.prioritized_components, self.segment_id_map)
    ...
    def optimize_heuristics(self, num_episodes):
        ...
        print(f"Best policy parameters: {self.best_rules}")
    ...
    def print_policy(self, num_episodes):
        ...
        print(f"Best policy parameters: {self.best_policy.policy_params}")
